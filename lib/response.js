...
class RESPONSE {
  ...
  constructor(app,request) {
    ...
    this.websocket = this._request.websocket
      ? this._request.websocket
      : null
  }
  ...
  async send(body) {
    ...
    if (this.websocket && this._state === 'done') {
      const _body = typeof body === 'string'
        ? JSON.parse(body)
        : body
      await this.websocket.setAction('complete').status(this._statusCode).send(_body)
    }
